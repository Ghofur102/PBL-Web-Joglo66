...
@section('content')
    <div class="w-full pb-12 mt-4">
        ...
        <x-tenant-card variant="flat" class="overflow-hidden mb-8">
            <div
                class="bg-gray-50 border-b border-gray-100 p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 -mx-4 -mt-4 mb-6">
                <div>
                    ...
                    <p class="text-gray-500 text-xs md:text-sm mt-2 font-mono">
                        No. Ref: <span
                            class="font-bold text-gray-800">{{ $booking->mainPayment->reference_id ?? 'Menunggu Pembayaran' }}</span>
                    </p>
                </div>

                <div class="flex flex-col items-start md:items-end gap-2">
                    <span
                        class="px-3 py-1 rounded-tenant-full text-xs font-bold border {{ $booking->badgeClass }} uppercase tracking-wider">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                <div class="bg-gray-50/50 rounded-tenant-lg p-5 border border-gray-100 flex flex-col justify-between">
                    ...
                    <div class="space-y-4 text-sm flex-1">
                        <div class="flex justify-between gap-4"><span class="text-gray-500 shrink-0">Nama Tim</span><span
                                class="font-semibold text-gray-900 text-right">{{ $booking->team_name }}</span></div>
                        <div class="flex justify-between gap-4"><span class="text-gray-500 shrink-0">No. HP</span><span
                                class="font-medium text-gray-900 text-right">{{ $booking->customer_phone ?? '-' }}</span>
                        </div>
                        <div class="flex justify-between gap-4"><span class="text-gray-500 shrink-0">Email</span><span
                                class="font-medium text-gray-900 text-right break-all">{{ $booking->customer_email ?? '-' }}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-4 text-sm">
                            <div class="flex justify-between gap-4"><span
                                    class="text-gray-500 shrink-0">Lapangan</span><span
                                    class="font-semibold text-gray-900 text-right">{{ $booking->field->name ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between gap-4"><span class="text-gray-500 shrink-0">Tgl
                                    Induk</span><span
                                    class="font-medium text-gray-900 text-right">{{ \Carbon\Carbon::parse($booking->booking_date)->format('d F Y') }}</span>
                            </div>
                        </div>
                    </div>

                    <div class="mt-4 pt-4 border-t border-gray-200/60">
                        <span class="block text-gray-400 text-xs font-bold uppercase tracking-wider mb-2">Catatan
                            Tambahan:</span>
                        <div
                            class="text-sm font-medium text-gray-800 bg-white p-3 rounded-tenant-md border border-gray-100 break-words whitespace-pre-wrap min-h-12">
                ...
                <table class="w-full text-left text-sm whitespace-nowrap min-w-full">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach ($booking->details as $detail)
                            <tr
                                class="hover:bg-gray-50/50 transition-colors {{ in_array($detail->detailStatus, ['cancelled', 'failed', 'expired']) ? 'opacity-60 bg-red-50/30' : '' }}">
                                <td class="px-6 py-4 text-gray-900">
                                    {{ \Carbon\Carbon::parse($detail->play_date)->format('d M Y') }}</td>
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="px-2.5 py-1 rounded-tenant-full text-xs font-semibold border {{ $detail->detailBadge }} capitalize">
                                        {{ $detail->detailStatus }}
                                    </span>
                                </td>
                                <td
                                    class="px-6 py-4 font-semibold {{ in_array($detail->detailStatus, ['cancelled', 'failed', 'expired']) ? 'line-through text-gray-400' : 'text-gray-900' }} text-right">
                            ...
                            <div class="relative pl-6 mt-6 space-y-6 before:absolute before:left-2 before:top-2 before:bottom-2 before:w-0.5 before:bg-gray-200">
                                <div class="relative">
                                    ...
                                    <p class="text-sm text-gray-700 mt-1">
                                        Jadwal dipesan: <strong>{{ \Carbon\Carbon::parse($detail->reschedules->last()->old_date ?? $detail->play_date)->format('d F Y') }}</strong>
                                        ...
                                        <div class="relative">
                                            ...
                                            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                                                <p class="text-xs font-bold text-amber-700 uppercase tracking-wider">
                                                    Tahap 2: Jadwal Diubah (Reschedule)
                                                </p>
                                            ...
                                            <div class="bg-amber-50/70 border border-amber-200/80 rounded-tenant-md p-3 text-xs text-amber-900 mt-2 space-y-1.5">
                                                ...
                                                {{-- Nominal Dana Masuk / Refund Reschedule --}}
            ...
            <div class="w-full text-sm bg-gray-50/50 p-6 rounded-tenant-lg border border-gray-200 shadow-tenant-sm mt-8">
                ...
                <div class="flex justify-between items-center mt-6 pt-6 border-t border-gray-200">
                    ...
                    @if (in_array($booking->overallStatus, ['cancelled', 'failed', 'expired']) && $booking->sisaTagihan == 0)
                        <span
                            class="px-4 py-2 bg-gray-100 text-gray-500 rounded-tenant-md font-black tracking-widest uppercase text-sm border border-gray-200 shadow-tenant-sm">
                            DIBATALKAN
                        </span>
                    @elseif($booking->sisaTagihan == 0)
                        <span
                            class="px-4 py-2 bg-green-100 text-green-700 rounded-tenant-md font-black tracking-widest uppercase text-sm border border-green-200 shadow-tenant-sm">
                            LUNAS
                        </span>
                    @else
